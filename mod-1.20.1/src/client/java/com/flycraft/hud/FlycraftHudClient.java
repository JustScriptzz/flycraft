...
public class FlycraftHudClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        BrainInstaller.ensureBrain();
        BrainLink.connect();
        HudRenderCallback.EVENT.register(BrainOverlay::render);
        ClientCommandRegistrationCallback.EVENT.register(
            (dispatcher, registryAccess) -> dispatcher.register(ClientCommandManager.literal("fly")
                ...
                .then(ClientCommandManager.literal("dope")
                    .then(ClientCommandManager.argument("amount", DoubleArgumentType.doubleArg(0, 10))
                        .executes(c -> send("!dope " + DoubleArgumentType.getDouble(c, "amount"))))
                    .executes(c -> send("!dope")))
                ...
                .then(ClientCommandManager.literal("summon")
                    .then(ClientCommandManager.argument("name", StringArgumentType.word())
                        .executes(c -> send("!summon " + StringArgumentType.getString(c, "name"))))
                    .executes(c -> send("!summon")))
                .then(ClientCommandManager.literal("play").executes(c -> send("!play")))
                .then(ClientCommandManager.literal("lan").executes(c -> {
                    LanOpener.announce();
                    return 1;
                }))
