...
LAST = {"logs": 0}
LAST_STEP = {"t": 0.0}  # last bot step (epoch); older than 5s = idle mode
HUDS = set()
SAVE_EVERY_EPS = 10
...
async def brain_state_msg(extra=None):
    s = brain.state_dict()
    live = time.time() - LAST_STEP["t"] < 5.0
    msg = {"type": "brain_state", "logs": LAST["logs"],
           "mode": "live" if live else "idle", **s}
    if extra:
        msg.update(extra)
...
async def broadcast():
    ...
    while True:
        ...
        if not HUDS:
            continue
        if time.time() - LAST_STEP["t"] > 5.0 and n % 2 == 0:
            # idle: spontaneous activity through the real network. A slowly
            # rotating visual bump drives the whole pathway in sequence;
            # zero reward, labeled "idle" — never training.
            d = (n // 10) % 8
            s = np.zeros(96)
            for r in range(3):
                for c in range(4):
